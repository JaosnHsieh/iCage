...
app.controller('cageCtrl', function ($scope, $filter, $q, $http, $rootScope, filterFilter,$timeout) {
  ...
  $scope.getCount = function (cageNo) {
    if (typeof $rootScope.animalsForCounting === 'undefined') {
      return;
    }
...
app.controller('animalInCageCtrl', function ($scope, $filter, $q, $http, $rootScope, filterFilter,$timeout) {


//// 暫時的即時料顯示 start
    $scope.stopPoll = true;
    $scope.poll = function () {
        $http.get('http://localhost:3000/api/sensor?ip='+$scope.cage.ip)
        .success(function (data) {
            $scope.sensor = {};  
            $scope.sensor.food = data.food;
            $scope.sensor.water = data.water;
            $scope.sensor.vibrate = data.vibrate;
            
            if(!$scope.stopPoll){ //關modal後 停止要sensor資料
    ...
    $scope.realtimeDataBtn = function () {
        $scope.stopPoll=false;
        $scope.poll();
        
        $('#realtimeData').modal('show');
    };
    //關modal後 停止要sensor資料
    $('#realtimeData').on('hidden.bs.modal', function () {
        $scope.stopPoll = true;
        });
